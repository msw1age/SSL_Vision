...
INITIAL_CHECKPOINT = "dino_phase2_epoch_19.pth"
LATEST_CHECKPOINT = "dino_phase4_latest.pth"
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, n_local_crops, global_crop_size=96, local_crop_size=64):
        ...
        moderate_jitter = T.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1)
...
class CUBEvalDataset(Dataset):
    # We print out k-NN validation accuracy during training
...
#                          4. MAIN LOOP
# ============================================================================
def train_dino_phase4(epochs=40, batch_size=512, n_local_crops=8):
    ...
    print(f"Total Images (iNat + Safe NABirds): {len(combined_dataset)}")

    dataloader = DataLoader(
        combined_dataset, batch_size=batch_size, shuffle=True,
        num_workers=os.cpu_count() - 8, pin_memory=True, drop_last=True, persistent_workers=True
    )
    ...
    for param in model.student_backbone.parameters():
    ...
    lr_warmup_sched = np.linspace(1e-6, lr_max_phase4, warmup_steps)
    lr_decay_sched = 1e-6 + 0.5 * (lr_max_phase4 - 1e-6) * (1 + np.cos(np.pi * np.arange(total_steps - warmup_steps) / (total_steps - warmup_steps)))
    lr_schedule = np.concatenate((lr_warmup_sched, lr_decay_sched))

    wd_schedule = np.full(total_steps, 0.06)

    momentum_schedule = np.full(total_steps, 0.9995)

    temp_schedule = np.full(total_steps, 0.05)
    ...
    if os.path.exists(LATEST_CHECKPOINT):
        print(f"Resuming Phase 4 from {LATEST_CHECKPOINT}...")
        checkpoint = torch.load(LATEST_CHECKPOINT, map_location=device, weights_only=False)
        start_epoch = checkpoint['epoch'] + 1
        real_model.student_backbone.load_state_dict(checkpoint['student_backbone'], strict=False)
        real_model.student_head.load_state_dict(checkpoint['student_head'])
        real_model.teacher_backbone.load_state_dict(checkpoint['teacher_backbone'], strict=False)
        real_model.teacher_head.load_state_dict(checkpoint['teacher_head'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        criterion.center = checkpoint['center']

    elif os.path.exists(INITIAL_CHECKPOINT):
        print(f"Loading Best Model from {INITIAL_CHECKPOINT}...")
        ...
        if 'center' in checkpoint: criterion.center = checkpoint['center']
        print(" > Weights loaded. Starting High-Energy Training at Epoch 0.")
    else:
        print(f"WARNING: Initial checkpoint {INITIAL_CHECKPOINT} not found. Starting from scratch (NOT RECOMMENDED).")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch+1) % 5 == 0:
            torch.save(save_dict, os.path.join(CHECKPOINT_DIR, f"phase4_epoch_{epoch}.pth"))
        ...
        try:
             os.system(f"scp -q {LATEST_CHECKPOINT} mlh9709@log-burst.hpc.nyu.edu:~/saved_checkpoints/")
        except: pass

if __name__ == "__main__":
    train_dino_phase4(epochs=40, batch_size=384, n_local_crops=4)
